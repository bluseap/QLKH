<%@ Page Title="" Language="C#" MasterPageFile="~/Shared/EOS.Master" AutoEventWireup="true" CodeBehind="NhapKHCDoc.aspx.cs" Inherits="EOSCRM.Web.Forms.KhachHang.NhapKHCDoc" %>
...
<asp:Content ID="head" ContentPlaceHolderID="headCPH" runat="server">
    <script type="text/javascript">
        ...
        function onSelectedIndexChangedEventHandler(txtMADPId, txtMADBId, txtCHISODAUId, txtCHISOCUOIId, hfGCSId, e) {
            ...
            var madp = $("#" + txtMADPId).val();
            var madb = $("#" + txtMADBId).val();
            ...
            if (savingMsg.value != "<%= DELIMITER.Passed %>") {
                setControlValue(txtMADPId, oldMADP);
                setControlValue(txtMADBId, oldMADB);
        ...
        function onKeyDownEventHandler(txtMADPId, txtMADBId, txtCHISODAUId, txtCHISOCUOIId, hfGCSId, order, e) {
            // get key code
            //if (!isValid)
            //    return;
            ...
            if (code == 13) {
                if (order == 1) {
                    var savingMsg = EOSCRM.Web.Common.AjaxCRM.InKhachHangMoiCD(idArr[0], idArr[3], csdau, cscuoi, idArr[8],
                        idArr[6], idArr[7], madp, madb);
                    ...
                    // move to next row
                    var madpId = getNextId(txtMADPId) + "";
                    if ($("#" + madpId).exists()) {
                    ...
                    $("#" + hfGCSId).val(val);
                        ...
                        // move to next row
                    var madbId = getNextId(txtMADBId) + "";
                    if ($("#" + madbId).exists()) {
            ...
            <div class="crmcontainer">
                <eoscrm:Grid 
                    ...
                    <Columns>
                        ...
                        <asp:TemplateField HeaderStyle-Width="4%" HeaderText="Danh số">
                            <ItemTemplate>
                        ...
                        <asp:TemplateField HeaderStyle-Width="150px" HeaderText="Mã đơn">
                            <ItemTemplate>
                                <asp:Label ID="txtMADDKMOI" Text='<%# new KhachHangDao().Get(Eval("IDKH").ToString()) != null ? new KhachHangDao().Get(Eval("IDKH").ToString()).MADDK.ToString() : Eval("IDKH").ToString() %>' runat="server" />                                    
                            </ItemTemplate>
                            <HeaderStyle Width="30px" />
                        </asp:TemplateField>        
                        <asp:TemplateField HeaderStyle-Width="30px" HeaderText="Mã DP">
                            <ItemTemplate>
                                <asp:TextBox ID="txtMADPKHM" Text='<%# Bind("MADP") %>' runat="server" />                                    
                            </ItemTemplate>
                            <HeaderStyle Width="30px" />
                        </asp:TemplateField>
                        <asp:TemplateField HeaderStyle-Width="30px" HeaderText="Mã DB">
                            <ItemTemplate>
                                <asp:TextBox ID="txtMADBKHM" Text='<%# Bind("MADB") %>' runat="server" />                                    
                            </ItemTemplate>
                            <HeaderStyle Width="30px" />
                        </asp:TemplateField>                                
                        <asp:BoundField HeaderStyle-Width="20%" HeaderText="Tên khách hàng" DataField="TENKH" />
                        ...
                        <asp:TemplateField HeaderText="Hoạt động" HeaderStyle-Width="30px">
						    <ItemTemplate>
							    <asp:LinkButton ID="btDelete" Text="Xóa" CommandName="DeleteKhachHang" 
							        CausesValidation="false" CommandArgument='<%# Eval("IDKH")%>' runat="server"
                                    Visible='<%# new KhachHangDao().Get(Eval("IDKH").ToString()) != null ? true : false %>' >
							    </asp:LinkButton>	
						    </ItemTemplate>
					    </asp:TemplateField>            
                    </Columns>
