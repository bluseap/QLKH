...
{
    public partial class KhoiTaoGhiChiSoLe : Authentication
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                Authenticate(Functions.GCS_KhoiTaoGhiChiSoLe, Permission.Read);
                PrepareUI();
        ...
        {
            Page.Title = Resources.Message.TITLE_GCS_KHOITAOKYGCSTHEODUONG;

            var header = (Header)Master.FindControl("header");
            if (header != null)
            {
                header.ModuleName = Resources.Message.MODULE_GHICHISO;
                header.TitlePage = Resources.Message.PAGE_GCS_KHOITAOKYGCSTHEODUONG;
            }
        }
        ...
        protected void btnKhoiTaoLe_Click(object sender, EventArgs e)
        {
            try
            {
            ...
            var dp = dpDao.Get(txtMADP.Text.Trim(), "");
            if(dp != null)
            {
                gcsDao.KhoiTaoGhiChiSo(date, dp);
                CloseWaitingDialog();
            }
            else
            {
