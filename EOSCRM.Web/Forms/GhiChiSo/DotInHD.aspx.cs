...
{
    public partial class DotInHD : Authentication
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                Authenticate(Functions.GCS_DotInHD, Permission.Read);
        ...
        {
            Page.Title = Resources.Message.TITLE_GCS_DOTINHD;

            var header = (Header)Master.FindControl("header");
            if (header != null)
            {
                header.ModuleName = Resources.Message.MODULE_GHICHISO;
                header.TitlePage = Resources.Message.PAGE_GCS_DOTINHD;
            }
        ...
        private void BindDuongPho()
        {
            ...
            var list = _dpDao.GetList(ddlKHUVUC.SelectedValue, txtKeywordDP.Text.Trim());
        ...
        private void BindData()
        {
            ...
            {
                var list = _dpDao.GetListKVDotIn(ddlKHUVUC.SelectedValue, ddlDOTGCS.SelectedValue);
            ...
            {
                var dp = _dpDao.Get(txtMADP.Text.Trim(), txtDUONGPHU.Text.Trim());
                if (dp == null) return;

                var list = _dpDao.GetListMADP(txtMADP.Text.Trim());
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {   
                ...
                if (!HasPermission(Functions.GCS_DotInHD, Permission.Insert))
                {
                    CloseWaitingDialog();
                ...
                if (!HasPermission(Functions.GCS_DotInHD, Permission.Update))
                {
                    CloseWaitingDialog();
                    ShowInfor(Resources.Message.WARN_PERMISSION_DENIED);
                    return;
        ...
        private void BindXemBaoCao()
        {
            try
            {
                ...
                var list = _lgcsDao.GetListKyKV(namF, thangF, ddlKHUVUC.SelectedValue);             
        ...
        private void XoaLichGCS(int idmadp)
        {
            try
            {
                ...
                var lgcs = _lgcsDao.Get(idmadp);
                
                Message msg2;
                msg2 = _dpDao.UpDotInInLichGCS2(lgcs.MADP, lgcs.IDMADOTINCU, CommonFunc.GetComputerName(), CommonFunc.GetLanIPAddressM(), b);

                Message msg;
                msg = _lgcsDao.DeleteLichGCS(idmadp, lgcs.MADP, CommonFunc.GetComputerName(), CommonFunc.GetLanIPAddressM(), "");
        ...
        private void BaoCaoLGCS()
        {
            try
            {
                ...
                int namF = Convert.ToInt16(txtNAM.Text.Trim());
                int thangF = Convert.ToInt16(ddlTHANG.SelectedValue);
                DataTable dt = _rpClass.BienKHNuoc("", ddlKHUVUC.SelectedValue, ddlDOTGCS.SelectedValue, "", thangF, namF, "DSLICHGCSN").Tables[0];
                ...
                string khuvuc = "XÍ NGHIỆP ĐIỆN NƯỚC " + _kvDao.Get(ddlKHUVUC.SelectedValue).TENKV.ToUpper();
                string tieude = "DANH SÁCH CHUYỂN LỊCH GHI CHỈ SỐ";
        ...
        //        DataTable dt = _rpClass.BienKHNuoc("", ddlKHUVUC.SelectedValue, ddlDOTGCS.SelectedValue, "", thangF, namF, "DSLICHGCSN").Tables[0];
        //        ReportDataSource rds = new ReportDataSource("DSLICHGCSN", dt);
        //        rvLICHGHICS.LocalReport.DataSources.Add(rds);                
                
        //        rvLICHGHICS.LocalReport.ReportPath = Server.MapPath("/Reports/GhiChiSo/DSDOTINHD.rdlc");
        ...
        //                    "Kỳ: " + thangF.ToString() + "/" + namF.ToString() + " (" + _dmdiDao.Get(_diDao.Get(ddlDOTGCS.SelectedValue).MADOTIN).TENDOTIN + ")";
        //        string ngaythang = "An Giang, ngày " + DateTime.Now.Day.ToString() + " tháng " + DateTime.Now.Month.ToString() + " năm " + DateTime.Now.Year.ToString();
        //        ReportParameter[] rpParams = new ReportParameter[]
        //        {
        //            new ReportParameter("rptxtKHUVUC", khuvuc),
        //            new ReportParameter("rptxtTIEUDE", tieude),
        //            new ReportParameter("rpKYGHICHISO", kyghi),
        //            new ReportParameter("rptxtNGAYTHANG", ngaythang)
        //        };
        //        rvLICHGHICS.LocalReport.SetParameters(rpParams);

        //        rvLICHGHICS.LocalReport.Refresh();                
