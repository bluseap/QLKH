...
{
    public partial class LapCTPower : Authentication
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                Authenticate(Functions.TK_LapChietTinhPo, Permission.Read);
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    if (Session["LAPCHIETTINH_MADDK"] != null && FromReport)
                    {
                        ...
                        var mp = _ddkpoDao.Get(MADDK);
                        var phuongList = _ppoDao.GetListKV(mp.MAKVPO);
                        if (mp.MAPHUONG != null) 
                        { 
                            //timkv(mp.MAPHUONG); 
        ...
        {
            Page.Title = Resources.Message.TITLE_TK_LAPCHIETTINHPO;

            var header = (Header)Master.FindControl("header");
            if (header != null)
            {
                header.ModuleName = Resources.Message.MODULE_THIETKE;
                header.TitlePage = Resources.Message.PAGE_TK_LAPCHIETTINHPO;
            }
        ...
        {
            var phuong = _ppoDao.Get(maphuong);
            ddlPHUONG.Items.Clear();
            ddlPHUONG.Items.Add(new ListItem(phuong.TENPHUONG, phuong.MAPHUONGPO));
        ...
        private void ChayChietTinh2(DONDANGKYPO ddkpo, NHANVIEN nv)
        {
            var ct = ctDao.Get(ddkpo.MADDKPO);
            if (ct == null)
            {
                var result = ctDao.CreateChietTinh2Po(ddkpo, CommonFunc.GetComputerName(), CommonFunc.GetLanIPAddressM(), nv.MANV);
                ...
                {
                    // Show message
                    ShowError("Chạy chiết tính điện không thành công.");
                }
            }
            else
            {
        ...
        private void ChayChietTinh(THIETKEPO tkpo, NHANVIEN nv)
        {
        ...
        private void ShowThongTinHeSoChietTinh(CHIETTINH chiettinh)
        {
            ...
            //lblDIACHI.Text = chiettinh.DONDANGKYPO.DIACHILD;
        ...
        private void BindDataForGrid()
        {
            try
            {
                ...
                var objList = _ddkpoDao.GetListForLapChietTinh(Keyword, FromDate, ToDate, kvpo.MAKVPO);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            /*
            qt.TENCT = txtTENCT.Text.Trim();
            qt.DIACHIHM = lblDIACHI.Text.Trim();
            qt.TENHM = txtTenHM.Text.Trim();
            qt.GHICHU = txtGhiChu.Text.Trim();

            qtdao.Update(qt, CommonFunc.GetComputerName(), CommonFunc.GetIpAdddressComputerName(), LoginInfo.MANV);
            */
            ...
            if (msg != null)
            {
                if (msg.MsgType != MessageType.Error)
                {
                    ...
                    Page.Response.Redirect(ResolveUrl("~") + "Forms/ThietKe/Power/LapCTPower.aspx", false);
                }
                else
                {
