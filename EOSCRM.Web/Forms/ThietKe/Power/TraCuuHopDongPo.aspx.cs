...
{
    public partial class TraCuuHopDongPo : Authentication
    {
        ...
        private HOPDONGPO HopDongPo
        {
            get
            {
                ...
                var hopdong = _hdpoDao.Get(txtMADDK.Text.Trim());
                if (hopdong == null)
                    return null;
                ...
                hopdong.MAPHUONGPO = String.IsNullOrEmpty(ddlPHUONG.SelectedValue) ? null : ddlPHUONG.SelectedValue;
                hopdong.MAKVPO = ddlKHUVUC.SelectedValue;
                hopdong.CODH = ddlKICHCODH.SelectedValue;
                hopdong.LOAIONG = txtLOAIONG.Text.Trim();
                hopdong.MAHTTT = ddlHTTT.SelectedValue;
                hopdong.MAMDSDPO = ddlMDSD.SelectedValue;
                ...
                hopdong.MADPPO = txtDANHSOHD.Text.Trim().Substring(0, 4);
                hopdong.DUONGPHUPO = "";
                hopdong.MADB = txtDANHSOHD.Text.Trim().Substring(4, 4);
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                Authenticate(Functions.TK_TraCuuHopDongPo, Permission.Read);
                PrepareUI();
        ...
        {
            Page.Title = Resources.Message.TITLE_TK_TRACUUHOPDONGPO;

            var header = (Header)Master.FindControl("header");
            if (header != null)
            {
                header.ModuleName = Resources.Message.MODULE_THIETKE;
                header.TitlePage = Resources.Message.PAGE_TK_TRACUUHOPDONGPO;
            }
        ...
        private void BindDataForGrid()
        {
            try
            {
                ...
                var kvpo =  _kvpoDao.GetPo(nhanvien.MAKV).MAKVPO;

                var list = _hdpoDao.GetList(MaDDK, SoHD, TenKH, SoNha, kvpo, FromDate, ToDate);
        ...
        {
            // load phuong
            var listPhuong = _ppoDao.GetList(makv);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            var maddkkh = _khpoDao.GetMADDK(hopdong.MADDKPO);
            if (maddkkh != null)
            {
                CloseWaitingDialog();
            ...
            var msg = _hdpoDao.Update(hopdong, CommonFunc.GetComputerName(), CommonFunc.GetLanIPAddressM(), LoginInfo.MANV);
        ...
        private void SetUpdateForm(HOPDONGPO obj)
        {
            ...
            txtDANHSOHD.Text = obj.MADPPO.ToString() + obj.MADB.ToString();
