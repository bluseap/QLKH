...
namespace EOSCRM.Web.Forms.ThietKe.Power
{
    public partial class QuanLyMauBVTPower : Authentication
    {
        ...
        private bool IsDataValid()
        {
            ...
            var existed = _mbvtDao.Get(txtMADDK.Text.Trim());
            if (existed != null)
            {
        ...
        {            
            ...
            if (UpdateMode.Equals(Mode.Create))
            {
                ...
                if (!HasPermission(Functions.TK_QuanLyMauBocVatTuPo, Permission.Insert))
                {
                    CloseWaitingDialog();
                ...
                // insert
                msg = _mbvtDao.Insert(don);
            }
            else
            {
                if (!HasPermission(Functions.TK_QuanLyMauBocVatTuPo, Permission.Update))
                {
                    CloseWaitingDialog();
                    ShowInfor(Resources.Message.WARN_PERMISSION_DENIED);
                    return;
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if ((strIds != null) && (!string.Empty.Equals(strIds)))
                {
                    ...
                    objs.AddRange(listIds.Select(ma => _mbvtDao.Get(ma)));

                    var msg = _mbvtDao.DeleteList(objs);
        ...
        protected void gvList_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                switch (e.CommandName)
                {
                    ...
                    case "DeleteItem":
                        // Authenticate
                        Authenticate(Functions.TK_QuanLyMauBocVatTuPo, Permission.Delete);

                        if (!string.Empty.Equals(id))
                        {
                            var mbvt = _mbvtDao.Get(id);
                            if (mbvt == null) return;

                            var msg = _mbvtDao.DeleteList(new List<MAUBOCVATTU> { mbvt });
