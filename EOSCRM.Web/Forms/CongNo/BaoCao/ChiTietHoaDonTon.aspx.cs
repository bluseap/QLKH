...
{
    public partial class ChiTietHoaDonTon : Authentication
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                Authenticate(Functions.CN_BaoCao_ChiTietHoaDonTon, Permission.Read);
                PrepareUI();
                ...
                {
                    var dt = (DataTable)Session[SessionKey.CN_BAOCAO_CHITIETHOADONTON];
                    Report(dt);
                }
            }
            catch (Exception ex)
        ...
        {
            Page.Title = Resources.Message.TITLE_CN_BAOCAO_CHITIETHOADONTON;

            var header = (Header)Master.FindControl("header");
            if (header != null)
            {
                header.ModuleName = Resources.Message.MODULE_CONGNO;
                header.TitlePage = Resources.Message.PAGE_CN_BAOCAO_CHITIETHOADONTON;
            }
        }
        ...
        {
            var ds =
                new ReportClass().ChiTietHoaDonTon(int.Parse(cboTHANG.Text.Trim()), 
        ...
        private void Report(DataTable dt)
        {
            ...
            var path = Server.MapPath("../../../Reports/CongNo/ChiTietHoaDonTon.rpt");
            rp.Load(path);
            ...
            Session[SessionKey.CN_BAOCAO_CHITIETHOADONTON] = dt;
