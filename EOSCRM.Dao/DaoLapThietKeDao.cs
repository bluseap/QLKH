...
namespace EOSCRM.Dao
{
    public  class DaoLapThietKeDao
    {
        ...
        public decimal? TTDaoLap(string madon)
        {
            var query = _db.DAOLAPTKs.Where(p => p.MADDK.Equals(madon) && p.LOAICP != "VC").Sum(p => p.THANHTIENCP);
        ...
        public decimal? TTVanChuyen(string madon)
        {
            var query = _db.DAOLAPTKs.Where(p => p.MADDK.Equals(madon) && p.LOAICP.Equals("VC")).Sum(p => p.THANHTIENCP);
