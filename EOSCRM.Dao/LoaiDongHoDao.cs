...
namespace EOSCRM.Dao
{
    public  class LoaiDongHoDao
    {
        ...
        public LoaiDongHoDao()
        {
            _db = new EOSCRMDataContext(Connectionstring);
        }
        
        public LOAIDH  Get(string ma)
        {
            return _db.LOAIDHs.Where(p => p.MALDH.Equals(ma)).SingleOrDefault();
        }
       

        public List<LOAIDH> Search(string key)
        {
            return _db.LOAIDHs.Where(p => p.MOTAKC.ToUpper().Contains(key.ToUpper())).ToList();
        }

        public List<LOAIDH> GetList()
        {
            return _db.LOAIDHs.ToList();
        }

        public List<LOAIDH> GetList(int fromIndex, int toIndex)
        {
            return GetList().Skip(fromIndex).Take(toIndex - fromIndex).ToList();
        }

        public List<LOAIDH> GetList(string keyword)
        {
            return _db.LOAIDHs.Where(p => (p.MALDH.ToLower().Contains(keyword.ToLower())
        ...
        public List<LOAIDH> GetListldh(string keyword)
        {
            return _db.LOAIDHs.Where(p => (p.MALDH.ToLower().Contains(keyword.ToLower())
                )).ToList();
        }
        ...
        public int Count( )
        {
            return _db.LOAIDHs.Count();
        }

        public Message Insert(LOAIDH objUi)
        {
            Message msg;
            try
            {
                ...
                _db.LOAIDHs.InsertOnSubmit(objUi);
                _db.SubmitChanges();
        ...
        public Message Update(LOAIDH objUi)
        {
            Message msg;
            try
            {
                // get current object in database
                var objDb = Get(objUi.MALDH   );
        ...
        public Message Delete(LOAIDH objUi)
        {
            Message msg;
            ...
            try
            {
                ...
                {
                    // error message
                    msg = new Message(MessageConstants.E_OBJECT_NOT_EXISTS, MessageType.Error, "Loại đồng hồ ", objUi.MALDH      );
                    return msg;
                }
                ...
                // Set delete info
                _db.LOAIDHs.DeleteOnSubmit(objDb);
                // Submit changes to db
                _db.SubmitChanges();
        ...
        /// <returns></returns>
        public Message DeleteList(List<LOAIDH> objList)
        {
            Message msg;
