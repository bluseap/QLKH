...
namespace EOSCRM.Dao
{
    public  class NhanVienDao
    {
        ...
        public List<NHANVIEN> SearchKV1(string key, string makv)
        {
        ...
        public List<NHANVIEN> SearchKVLX(string key, string makv, string mapb)
        {            
            return _db.NHANVIENs.Where(p => (p.MAKV.Equals(makv) && p.MAPB.Equals(mapb))
                        && p.HOTEN.ToUpper().Contains(key.ToUpper()))
                    .OrderBy(p => p.HOTEN)
                    .OrderBy(p => p.MACV)
                    .OrderBy(p => p.MAPB)
                    .ToList();
        ...
        public List<NHANVIEN> SearchKV3(string key, string makv, string macv)
        {
            //return _db.NHANVIENs.Where(p => (p.MAKV.Equals(makv) && p.HOTEN.ToUpper().Contains(key.ToUpper())
            return _db.NHANVIENs.Where(p => p.MAKV.Equals(makv) && 
                (p.HOTEN.ToUpper().Contains(key.ToUpper()) || p.MANV.ToUpper().Contains(key.ToUpper()))
                //|| p.PHONGBAN.TENPB.ToUpper().Contains(key.ToUpper())
                //|| p.CONGVIEC.TENCV.ToUpper().Contains(key.ToUpper())
                )
        ...
        public List<NHANVIEN> SearchKV_GD(string key, string makv, string macv)
        {            
            return _db.NHANVIENs.Where(p => p.MAKV.Equals(makv) && 
                    (p.HOTEN.ToUpper().Contains(key.ToUpper()) || p.MANV.ToUpper().Contains(key.ToUpper()))
                            && (p.MACV == "GD" || p.MACV == "PGD")          
                )  
                    .OrderBy(p => p.HOTEN)
                    .OrderBy(p => p.MACV)
                    .OrderBy(p => p.MAPB)
        ...
        public List<NHANVIEN> SearchTP_PT(string key, string makv, string macv)
        {           
            return _db.NHANVIENs.Where(p => p.MAKV.Equals(makv) && (p.HOTEN.ToUpper().Contains(key.ToUpper())
                            || p.MANV.ToUpper().Contains(key.ToUpper()))
                            && (p.MACV == "TP" || p.MACV == "PT")               
        ...
        public List<NHANVIEN> SearchKVPB(string key, string makv, string mapb)
        {
            //return _db.NHANVIENs.Where(p => (p.MAKV.Equals(makv) && p.HOTEN.ToUpper().Contains(key.ToUpper())
            return _db.NHANVIENs.Where(p => p.MAKV.Equals(makv) && p.MAPB.Equals(mapb) && (p.HOTEN.ToUpper().Contains(key.ToUpper())
                            || p.MANV.ToUpper().Contains(key.ToUpper()))
                //|| p.PHONGBAN.TENPB.ToUpper().Contains(key.ToUpper())
                //|| p.CONGVIEC.TENCV.ToUpper().Contains(key.ToUpper())
                )
        ...
        public List<NHANVIEN> TimNV_NM_To(string key, string makv, string macv)
        {
            return _db.NHANVIENs.Where(p => (p.MAKV.Equals(makv) && (p.MAPB.Equals("NB") || p.MAPB.Equals("TA") || p.MAPB.Equals("TD")))// && p.HOTEN.ToUpper().Contains(key.ToUpper())
                //|| p.MANV.ToUpper().Contains(key.ToUpper())
                //|| p.PHONGBAN.TENPB.ToUpper().Contains(key.ToUpper())
                //|| p.CONGVIEC.TENCV.ToUpper().Contains(key.ToUpper())
                //)
        ...
        public List<NHANVIEN> TimNVKyThuat(string key, string makv, string macv)
        {
            return _db.NHANVIENs.Where(p => (p.MAKV.Equals(makv) && p.MAPB.Equals("KTDN"))// && p.HOTEN.ToUpper().Contains(key.ToUpper())
                //|| p.MANV.ToUpper().Contains(key.ToUpper())
                //|| p.PHONGBAN.TENPB.ToUpper().Contains(key.ToUpper())
                //|| p.CONGVIEC.TENCV.ToUpper().Contains(key.ToUpper())
                //)
        ...
        public Message Update(NHANVIEN objUi)
        {
            ...
            try
            {
                ...
                if (objDb != null)
                {
                    ...
                    objDb.MAHINHNV = objUi.MAHINHNV;
                    objDb.HINHNVLL = objUi.HINHNVLL;
        ...
        public string NewId()
        {
            var mact = DateTime.Now.ToString("WP");
            var query = _db.NHANVIENs.Max(p => p.MANV);

            if (query != null)
            {
                var temp = int.Parse(query.Substring(2,6)) + 1;
                query = mact + temp.ToString("D6");
            }
            else
            {
                query = mact + "000001";
