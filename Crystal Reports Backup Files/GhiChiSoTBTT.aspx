<%@ Page Title="" Language="C#" MasterPageFile="~/Shared/EOS.Master" AutoEventWireup="true" CodeBehind="GhiChiSoTBTT.aspx.cs" Inherits="EOSCRM.Web.Forms.GhiChiSo.GhiChiSoTBTT" %>
        ...
        function onKeyDownEventHandler(txtCHISODAUId, txtCHISOCUOIId, txtKLTIEUTHUId, ddlTTHAIGHIId, hfGCSId, order, e  ) {
            ...
            if (code == 13) {
                if (order == 1) {
                    ...
                    var csd = $("#" + txtCHISODAUId).val();
                    var csc = $("#" + txtCHISOCUOIId).val();
                    if (csc.toLowerCase() != "g") {
                        if (csc.toLowerCase() != "i") {
                            if (csc.toLowerCase() != "d") {
                                if (csc.toLowerCase() != "n") {
                                    if (csc.toLowerCase() != "l") {
                                        if (csc.toLowerCase() != "c") {
                                            if (csc.toLowerCase() != "h") {
                                                if (csc.toLowerCase() != "m") {
                                                    if (csc.toLowerCase() != "t") {
                    ...
                    if (tthaighi != "Q") {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "EOSCRM.Web.Common.AjaxCRM.cs/UpdateGCSTBTT",
                            data: '{idArr[0], idArr[1], idArr[2], csd.toString(), csc.toString(), mttCu.toString(), mtt.toString(), idArr[7], tthaighi, idArr[6]}',
                            contentType: "json"
                        });
                    ...
                    if (tthaighi != "Q") {
                        ...
                        var csc = $("#" + txtCHISOCUOIId).val();
                        var kltt = csc - csd;
                        if (kltt < 0 || parseInt(oldKltt_1) + 30 < kltt) {
                            //setControlValue(txtCHISOCUOIId, oldCsc);
                            //setControlValue(txtKLTIEUTHUId, oldKltt);
                            alertWithFocusSelect(msg3, txtCHISOCUOIId);
                            //return;
                        ...
                        var savingMsg = EOSCRM.Web.Common.AjaxCRM.UpdateGCSTBTT(idArr[0], idArr[1], idArr[2], csd.toString(), csc.toString(), mtt.toString(), kltt.toString(), idArr[7], tthaighi, idArr[6]);
        ...
        <ContentTemplate>
            ...
            <div class="crmcontainer" id="divList" runat="server" visible="false">
                <eoscrm:Grid ID="gvList" runat="server" UseCustomPager="true" PageSize="350"  
                    OnRowDataBound="gvList_RowDataBound" OnPageIndexChanging="gvList_PageIndexChanging" OnSelectedIndexChanged="gvList_SelectedIndexChanged">
                    ...
                    <Columns>
                        ...
                        <asp:TemplateField HeaderStyle-Width="70px" HeaderText="CS mới">
                            <ItemTemplate>
                                <asp:TextBox ID="txtCHISOCUOI" Text='<%# Bind("CHISOCUOI") %>' 
                                    CssClass='<%# Eval("TTHAIGHI").ToString() == "THAY" ? "readonly" : "editable" %>'
                                    ReadOnly='<%# Eval("TTHAIGHI").ToString() == "THAY" ? true : false %>' 
                                    runat="server" />
                            </ItemTemplate>
                            <HeaderStyle Width="70px" />
                        </asp:TemplateField>
                        <asp:TemplateField HeaderStyle-Width="115px" HeaderText="Trạng thái ghi">
                            <ItemTemplate>
                                <asp:DropDownList ID="ddlTTHAIGHI" Width="110px" SelectedValue='<%# Bind("TTHAIGHI") %>' runat="server">
                                    ...
                                    <asp:ListItem Value="A" Text="A_Khách hàng cung cấp" />
                                </asp:DropDownList>
                            </ItemTemplate>
